...
static void test_no_op(void) {
  ...
  const grpc_channel_args* args = grpc_core::CoreConfiguration::Get()
                                      ...
                                      .ToC();
  ASSERT_EQ(GRPC_ERROR_NONE, grpc_tcp_server_create(nullptr, args, &s));
...
static void test_connect(size_t num_connects,
  ...
  ASSERT_TRUE(svr_port > 0);
  /* Cannot use wildcard (port==0), because add_port() will try to reuse the
     same port as a previous add_port(). */
  svr1_port = grpc_pick_unused_port_or_die();
  ASSERT_TRUE(svr1_port > 0);
  ...
  ASSERT_TRUE(grpc_tcp_server_port_fd(s, 2, 0) < 0);

  /* Bad fd_index. */
  ASSERT_TRUE(grpc_tcp_server_port_fd(s, 0, 100) < 0);
  ASSERT_TRUE(grpc_tcp_server_port_fd(s, 1, 100) < 0);

  /* Got at least one fd per port. */
  svr_fd_count = grpc_tcp_server_port_fd_count(s, 0);
  ASSERT_TRUE(svr_fd_count >= 1);
  svr1_fd_count = grpc_tcp_server_port_fd_count(s, 1);
  ASSERT_TRUE(svr1_fd_count >= 1);
    ...
    for (port_num = 0; port_num < num_ports; ++port_num) {
      ...
      for (fd_num = 0; fd_num < num_fds; ++fd_num) {
        ...
        ASSERT_EQ(getsockname(fd, (struct sockaddr*)dst.addr.addr,
                  ...
                  0);
        ASSERT_TRUE(dst.addr.len <= sizeof(dst.addr.addr));
        test_addr_init_str(&dst);
  ...
  ASSERT_TRUE(grpc_tcp_server_port_fd(s, 0, 0) >= 0);
