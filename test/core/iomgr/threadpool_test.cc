...
TEST(ThreadpoolTest, TestConstructorOption) {
  ...
  ASSERT_EQ(pool->thread_options().stack_size(), options.stack_size());
  delete pool;
}
...
TEST(ThreadpoolTest, TestAdd) {
  ...
  delete pool;
  ASSERT_EQ(functor->count(), kThreadSmallIter);
  delete functor;
...
TEST(ThreadpoolTest, TestMultiAdd) {
  ...
  delete pool;
  ASSERT_EQ(functor->count(), kThreadLargeIter * num_work_thds);
  delete functor;
  gpr_log(GPR_DEBUG, "Done.");
}
...
class SimpleFunctorCheckForAdd : public grpc_completion_queue_functor {
 public:
  ...
  static void Run(struct grpc_completion_queue_functor* cb, int /*ok*/) {
    ...
    (*callback->count_)++;
    ASSERT_EQ(*callback->count_, callback->internal_success);
  }
