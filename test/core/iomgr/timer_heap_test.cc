...
static void check_valid(grpc_timer_heap* pq) {
  ...
  for (i = 0; i < pq->timer_count; ++i) {
    ...
    if (left_child < pq->timer_count) {
      ASSERT_TRUE(pq->timers[i]->deadline <= pq->timers[left_child]->deadline);
    }
    if (right_child < pq->timer_count) {
      ASSERT_TRUE(pq->timers[i]->deadline <= pq->timers[right_child]->deadline);
    }
  }
}
...
static void shrink_test(void) {
  ...
  ASSERT_TRUE(pq.timer_count * 2 <= pq.timer_capacity);
  ASSERT_TRUE(pq.timer_capacity <= pq.timer_count * 4);
  check_valid(&pq);
  ...
  while (pq.timer_count > 0) {
    ...
    if (pq.timer_count >= 8) {
      ...
      ASSERT_TRUE(16 <= pq.timer_capacity);
    }
    check_valid(&pq);
  }

  ASSERT_EQ(0, pq.timer_count);
  ASSERT_TRUE(pq.timer_capacity >= 16 && pq.timer_capacity < 32);
